...
let questions = [
  {
    ...
    description:
      "I've been trying to center a div element horizontally and vertically on the page. What are the best methods to achieve this?",
  ...
  },
  {
    ...
    description:
      "Can someone explain the difference between let, const, and var with examples?",
    tags: ["javascript", "es6"],
    author: "devGuru",
  ...
  },
  {
    ...
    description:
      "I'm having trouble understanding how to properly use async/await in Node.js. Can someone provide examples?",
...
let answers = [
  {
    ...
    questionId: 1,
    content:
      "You can use flexbox: `display: flex; justify-content: center; align-items: center;` on the parent container.",
    author: "flexMaster",
  ...
  },
  {
    ...
    questionId: 1,
    content:
      "Another approach is using CSS Grid: `display: grid; place-items: center;` on the parent.",
    author: "gridGuru",
  ...
  },
  {
    ...
    questionId: 2,
    content:
      "`var` is function-scoped, while `let` and `const` are block-scoped. `const` is used for variables that won't be reassigned.",
    author: "jsNinja",
  ...
  },
  {
    ...
    questionId: 3,
    content:
      "Use try-catch blocks with async/await: `try { const result = await someAsyncFunction(); } catch (error) { console.error(error); }`",
    author: "asyncExpert",
...
function updateQuestionStats(questionId) {
  ...
  if (question) {
    ...
    question.accepted = answers.some(
      (a) => a.questionId === questionId && a.accepted,
    );
  }
}
...
app.get("/api/questions", (req, res) => {
  ...
  console.log(
    `[${new Date().toISOString()}] Returning ${filteredQuestions.length} questions (filtered from ${questions.length})`,
  );
...
app.get("/api/questions/:id", (req, res) => {
  ...
  question.views++;
  console.log(
    `[${new Date().toISOString()}] Question ${id} viewed. New view count: ${question.views}`,
  );
...
app.post("/api/questions", (req, res) => {
  ...
  console.log(
    `[${new Date().toISOString()}] New question created: "${newQuestion.title}" by ${newQuestion.author}`,
  );
...
app.post("/api/questions/:id/answers", (req, res) => {
  ...
  if (question.author !== author) {
    ...
    console.log(
      `[${new Date().toISOString()}] Notification created: ${author} answered ${question.author}'s question`,
    );
  }

  console.log(
    `[${new Date().toISOString()}] New answer created by ${author} for question ${questionId}`,
  );
...
app.listen(PORT, () => {
  const timestamp = new Date().toISOString();
  console.log(
    `[${timestamp}] 🚀 StackIt Simple Server running on port ${PORT}`,
  );
  console.log(`[${timestamp}] 📚 API Documentation: http://localhost:${PORT}`);
  console.log(`[${timestamp}] 🔍 Sample endpoints:`);
  console.log(`[${timestamp}]    GET  http://localhost:${PORT}/api/questions`);
  console.log(
    `[${timestamp}]    GET  http://localhost:${PORT}/api/notifications`,
  );
  console.log(`[${timestamp}]    GET  http://localhost:${PORT}/api/health`);
  console.log(
    `[${timestamp}] 📊 Initial data loaded: ${questions.length} questions, ${answers.length} answers, ${notifications.length} notifications`,
  );
});
